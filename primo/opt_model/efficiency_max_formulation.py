...
@declare_custom_block("MaxFormulationBlock", rule="compute_metric_score")
class MaxFormulationBlockData(BlockData):
    ...
    def compute_metric_score(
        ...
        *args,
        ...
        norm_metric_data = {
            w: min(float(value), 1.0) for w, value in norm_metric_data.items()
        }
...
def build_cluster_efficiency_model(eff_blk):
    ...
    for metric in WELL_BASED_METRICS:
        ...
        setattr(
            ...
            MaxFormulationBlock(
                ...
                metric_data=wd.data.loc[list(cm.set_wells), col_name],
                scaling_factor=getattr(sf, "max_" + metric),
                metric_type="well_based",
            ),
        )

    pairwise_metrics = pm.model_inputs.pairwise_metrics[cm.index()]
    ...
    if weights.num_wells > 0:
        # pylint: disable = undefined-variable
        metric = "num_wells"
        eff_blk.num_wells = MaxFormulationBlock(
            weight=getattr(weights, metric),
    ...
    if weights.num_unique_owners > 0:
        ...
        eff_blk.num_unique_owners = MaxFormulationBlock(
            weight=getattr(weights, metric),
            metric_data=pd.Series([0, 0]),
            scaling_factor=getattr(sf, "max_" + metric),
            metric_type=metric,
