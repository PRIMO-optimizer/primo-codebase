...
class SetOfMetrics:
    ...
    def __init__(
        self,
        supported_metrics: Optional[Dict[str, _SupportedContent]] = None,
        basis: int = 100,
    ):
        if not isinstance(basis, int):
            raise_exception(
                f"Metric basis must be an integer. Received {basis}.", TypeError
            )

        if basis <= 0:
            raise_exception(
                f"Metric basis must be a positive integer. Received {basis}.",
                ValueError,
            )
    ...
    def __iter__(self):
        ...
        return iter(obj for obj in self.__dict__.values() if isinstance(obj, Metric))
    ...
    def __str__(self) -> str:
        ...
        output = pd.DataFrame(data, columns=list(data.keys()), index=_index).to_string()
        ...
        for obj in self.get_primary_metrics.values():
            ...
            output += pd.DataFrame(
                data, columns=list(data.keys()), index=_index
            ).to_string()
    ...
    def items(self):
        ...
        """
        return {k: v for k, v in self.__dict__.items() if isinstance(v, Metric)}.items()
    ...
    def check_validity(self):
        ...
        if not np.isclose(primary_metric_sum, basis):
            raise_exception(
                f"Sum of weights of primary metrics does not add up to {basis}",
                ValueError,
            )
...
class ImpactMetrics(SetOfMetrics):
    ...
    def __init__(
        ...
        super().__init__(impact_metrics, basis=basis)
