...
def get_well_data_cols_fixture():
    ...
    im_mt.delete_metric("other_losses")
...
def test_required_data():
    ...
    col_names = WellDataColumnNames(
        well_id="C1", latitude="C2", longitude="C3", age=None, depth=None
    )
    for metric in im_metrics:
        if metric._required_data is not None:
            assert metric._required_data in col_names
