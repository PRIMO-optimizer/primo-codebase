...
def test_drop_wells(caplog, get_well_data_from_csv):
    """Tests drop_wells method"""
    wd = get_well_data_from_csv

    # Test the ValueError error message
    with pytest.raises(ValueError, match="Well\\(s\\) not found in the dataset"):
        wd.drop_wells(wells=[125000, 135000], reason="test")

    # Test the method
    wd.drop_wells(wells=[47, 48, 49, 50, 51], reason="test")
    assert "Removed 5 wells from the dataset." in caplog.text
    # pylint: disable = protected-access
    assert "test" in wd._removed_rows
