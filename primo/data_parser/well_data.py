...
from primo.data_parser.default_data import CONVERSION_FACTOR, MISSING_INPUT_DATA_COLUMNS
...
class WellData:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(data, pd.DataFrame):
            ...
            for col in column_names.values():
                if col not in self.data.columns:
                    raise_exception(
                        f"Column '{col}' is missing from the input data.", ValueError
                    )
        ...
        msg = (
            ...
            f"wells ({perc_wells_removed}% wells in the input data set) \n\tbecause of missing "
            f"information. To include these wells in the analysis, \n\tplease provide the missing "
            f"information. \n\tList of wells removed can be queried using the get_removed_wells "
            f"\n\tand get_removed_wells_with_reason properties."
        )
        if num_wells_processed < num_wells_input:
            LOGGER.warning(msg)

        if num_wells_processed == 0:
            msg = "All wells in the data set have been removed during preliminary processing."
            for reason, wells in self._removed_rows.items():
                msg += f"\n{len(wells)} wells were removed for {reason}."
    ...
    def _repr_html_(self):
        ...
        cols = self.get_essential_columns + self.get_priority_score_columns
        cols = list(dict.fromkeys(cols))
        return self.data[cols].to_html(index=False)
    ...
    @property
    def get_maximum_priority_score(self) -> float:
        """
        Returns the maximum priority score of the wells without any normalization
        """
        cn = getattr(self._col_names, "priority_score", None)
        if cn is None:
            LOGGER.warning("Returning None, since priority scores are not available!")
            return None
        return self.data[cn].max()
    ...
    def get_high_priority_wells(self, num_wells: int, sort_by_priority: bool = True):
        ...
        ascending = self.config.priority_score_reverse

        hp_wells = self._construct_sub_data(
            self.data.sort_values(cn, ascending=ascending)
            .head(num_wells)
            .index.to_list()
        )
        ...
        if sort_by_priority:
            ...
            hp_wells.data = hp_wells.data.sort_values(cn, ascending=ascending)
    ...
    def drop_wells(self, wells: list[int], reason: str):
        """
        Drops specified list of wells from the dataset

        Parameters
        ----------
        wells : list[int]
            List of well indices that need to be dropped
        reason : str
            Reason for dropping these wells
        """
        # Remove wells from the dataset
        try:
            self.data = self.data.drop(wells)
            LOGGER.warning(f"Removed {len(wells)} wells from the dataset.")
        except KeyError as excp:
            raise ValueError("Well(s) not found in the dataset") from excp

        # Store removed well indices
        self._removed_rows[reason] = wells

        # Remove wells from well-types
        for wt, val in self._well_types.items():
            if val is not None:
                self._well_types[wt] -= set(wells)
    ...
    def _check_input_data_availability(self, column: str):
        ...
        if missing_method == "fill":
            ...
            self.drop_incomplete_data(col_name=col_name, dict_key=f"missing {column}")
    ...
    def _filter_production_volume(self):
        ...
        filters = {
            "gas": {
                "col": wcn.life_gas_production,
                "min": self.config.min_lifetime_gas_production,
                "max": self.config.max_lifetime_gas_production,
            },
            "oil": {
                "col": wcn.life_oil_production,
                "min": self.config.min_lifetime_oil_production,
                "max": self.config.max_lifetime_oil_production,
            },
        }

        if all(well_type["col"] is None for well_type in filters.values()):
            LOGGER.warning(
        ...
        for well_type, filter_info in filters.items():
            col = filter_info["col"]
            if col is not None:
                bounds = [
                    (
                        filter_info["min"],
                        "below the minimum",
                        lambda col, value: self.data[col] <= value,
                    ),
                    (
                        filter_info["max"],
                        "above the maximum",
                ...
                for bound, label, condition_func in bounds:
                    if bound is not None:
                        removed_wells = self.data[condition_func(col, bound)].index

                        if len(removed_wells) > 0:
                            reason = (
                                f"lifetime {well_type} production {label} threshold"
                            )
                            self._removed_rows[reason] = removed_wells.to_list()
                            self.data = self.data.drop(index=removed_wells)

                            LOGGER.warning(
                                f"Some wells have been removed for being {label} specified "
                                f"lifetime {well_type} production volume."
                            )

    def _filter_ann_production(self):
        """Removes wells whose annual production rate exceeds the threshold value"""
        try:
            total_boe_produced = (
                self[self._col_names.ann_gas_production] / CONVERSION_FACTOR
            ) + self[self._col_names.ann_oil_production]
        except KeyError:
            # Data needed to filter wells is not available, so return
            LOGGER.warning(
                "Unable to filter wells based on threshold_daily_production_boe "
                "because the required data is not available."
            )
            return

        overproduction_wells = total_boe_produced[
            total_boe_produced > (self.config.threshold_daily_production_boe * 365)
        ].index.to_list()

        if len(overproduction_wells) > 0:
            LOGGER.warning(
                f"Found {len(overproduction_wells)} wells whose annual production exceeds "
                f"{self.config.threshold_daily_production_boe} BOE per day."
            )
            self.drop_wells(
                wells=overproduction_wells,
                reason="exceeding daily production threshold",
            )
    ...
    def _categorize_shallow_deep(self):
        ...
        if wt_col_name in self:
            ...
            self.fill_incomplete_data(
                ...
                flag_col_name=wt_col_name + "_flag",
            )
    ...
    def _process_input_data(self):
        ...
        self.drop_incomplete_data(col_name=wcn.well_id, dict_key="missing well_id")
        ...
        self.drop_incomplete_data(col_name=wcn.latitude, dict_key="missing latitude")
        self.drop_incomplete_data(col_name=wcn.longitude, dict_key="missing longitude")

        # Drop wells if the operator name is not available when well owner information
        # is provided
        unknown_owner = []
        if self.config.verify_operator_name:
            ...
            self.drop_incomplete_data(
                col_name=wcn.operator_name, dict_key="missing operator_name"
            )
            ...
            if len(unknown_owner) > 0:
                ...
                self._removed_rows["unknown operator_name"] = unknown_owner

        # Check if input data is missing
        # Only check missing data if project comparison is not selected
        if not self.config.scenario_type.project_comparison:
            for col in MISSING_INPUT_DATA_COLUMNS:
                self._check_input_data_availability(column=col)

        # Check if the priority score is provided and any score is missing
        # when well ranking is performed
        if not self.config.scenario_type.well_ranking:
            ...
            flag, empty_cell = self.has_incomplete_data(wcn.priority_score)
            well_empty_cell = self.data.loc[empty_cell, wcn.well_id]
            if flag:
                raise_exception(
        ...
        if self.config.threshold_daily_production_boe is not None:
            self._filter_ann_production()

        # Categorize wells: Shallow/Deep, Oil/Gas
        self._categorize_oil_gas()
        self._categorize_shallow_deep()

        # Check if plugging cost is provided by the user
        if wcn.cost_of_plugging is not None:
            self._check_plugging_cost_data()
        ...
        # Only perform the check when project comparison is not selected
        if not self.config.scenario_type.project_comparison:
    ...
    def _process_data(self, metrics: SetOfMetrics):
        ...
        for metric in metrics:
            ...
            if metric.fill_missing_value is not None:
                fill_in_value = metric.fill_missing_value
            # check incomplete for custom impact factors
            else:
                if metric.is_binary_type:
                    fill_in_value = metric.has_inverse_priority
                else:
                    data_col = self.data[metric.data_col_name]
                    if metric.has_inverse_priority:
                        fill_in_value = data_col.max()
                    else:
                        fill_in_value = data_col.min()

            self.fill_incomplete_data(
                col_name=metric.data_col_name,
                value=fill_in_value,
                flag_col_name=metric.data_col_name + "_flag",
            )
    ...
    def _check_plugging_cost_data(self):
        """
        Perform preliminary checks on plugging cost data.
        Fill missing data with default value or maximum value.
        """
        wcn = self._col_names
        if self.data[wcn.cost_of_plugging].isna().all():
            LOGGER.warning(
                "Found empty cost_of_plugging column in data."
                "Ignoring this column in WellData."
            )
            wcn.cost_of_plugging = None
            return

        if (self.data[wcn.cost_of_plugging] < 0).any():
            negative_rows = list(self.data[self.data[wcn.cost_of_plugging] < 0].index)
            raise_exception(
                f"Plugging cost per well must be non-negative. Found negative "
                f"values in rows {negative_rows}.",
                ValueError,
            )
        if self.data[wcn.cost_of_plugging].isna().any():
            ...
            self.fill_incomplete_data(
                ...
                flag_col_name=wcn.cost_of_plugging + "_flag",
            )
            LOGGER.warning(
    ...
    def compute_priority_scores(self):
        ...
        the total priority score, or normalize the priority score provided by users.
        This method must be called after processing the data for custom metrics (if any).
        """
        ...
        # user provided priority scores
        if hasattr(self._col_names, "priority_score"):
            col_name = self._col_names.priority_score
            data_col = self.data[col_name]
            reverse = self.config.priority_score_reverse

            max_score = data_col.max()
            min_score = data_col.min()

            if min_score < 0:
                raise_exception(
                    "Priority scores must be >= 0. Please check "
                    "your data for negative priority scores assigned to wells.",
                    ValueError,
                )

            # Handle the case where user-provided priority scores are the same
            # for all wells
            if np.isclose(max_score, min_score, rtol=0.001):
                # if all priority scores are 0, then they stay at 0
                normalized_value = 100 if max_score > 1e-6 else 0
                scaled = pd.Series(normalized_value, index=data_col.index)
                if (
                    self.config.scenario_type.project_recommendation
                    and not self.config.scenario_type.well_ranking
                ) or (
                    self.config.scenario_type.project_comparison
                    and not self.config.scenario_type.well_ranking
                ):
                    LOGGER.warning(
                        "The provided priority scores are identical across all wells."
                    )
            # Rather than normalizing priority scores as we normalize data
            # we make an assumption that 0 is the lowest possible priority score
            # in the normal case and that 0 is the highest possible priority score
            # in the reverse case
            # we now scale the priority score even if it is less than 100
            else:
                # the full range of the data is the largest score in the dataset
                # assuming that 0 is the other bound
                scaling_factor = max_score

                scaled = (
                    (1 - (data_col / scaling_factor)) * 100
                    if reverse
                    else data_col / scaling_factor * 100
                )

                LOGGER.info("Normalize priority score to 0-100.")
            ...
            basis = self.config.impact_metrics.basis

            self.add_new_column_ordered(
                "priority_score",
                f"Priority Score [0-{basis}]",
            ...
            if basis != 100:
                ...
                self.add_new_column_ordered(
                    "priority_score_normalized",
                    "Priority Score [0-100]",
                    self.data[self._col_names.priority_score] / basis * 100.0,
                )
    ...
    def add_well_rank(self):
        ...
        if hasattr(self._col_names, "priority_score"):
            ...
            sorted_rank = (
                self.data[self._col_names.priority_score]
    ...
    def has_fully_incomplete_data(self, col_name: str):
        """
        Checks if a column contains all empty cells.
        ...
        -------
        bool
            True, if the column has all empty cells; False, otherwise.

        """
        if self.data[col_name].isna().all():
            LOGGER.warning(f"Column: {col_name} has all empty cells")
            return True
