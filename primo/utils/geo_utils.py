...
def get_distance(
    ...
    is_valid_geopoint(destination)

    unit = Unit.MILES if distance_unit == "MILES" else Unit.KILOMETERS
    return haversine(origin, destination, unit)
...
def get_nearest_neighbors(
    ...
    ball_tree, points_radians = build_ball_tree(points)

    # Query using the cutoff (already in miles or needs unit conversion before this)
    neighbors = [
