...
class UserPriorities:
    ...
    config_dict : dict
        A dictionary that determines the parameters for the widget to be displayed.
        An example structure includes:
        config_dict = {"Well History": {"default": 20,
        "sub_weights":
        {"Leak [Yes/No]": {"default": 40}},
        "Compliance [Yes/No]" : {"default": 60}}
        }
...
class BaseSelectWidget:
    ...
    def return_selections(self):
        """
        Return the list of project selections made by the user
        """
...
class SubSelectWidget(BaseSelectWidget):
    ...
    def _on_change(self, data) -> None:
        ...
        well_candidate = self.wd.data[
            self.wd.data[self.wd.col_names.cluster] == int(cluster)
        ][self.wd.col_names.well_id]
    ...
    def return_selections(self):
        """
        Return the list of well selections made by the user
        """
...
class SelectWidgetAdd(SelectWidget):
    ...
    well_choices : WellData
        The WellData object for wells that are candidates for the "Add Well"
        field of the add widget. This includes wells that are not currently
        selected plug wells removed.
    ...
    well_data : WellData
        The WellData object of the entire well dataset to identify available project
        candidates for the "To Project" field of the add widget
    ...
    def __init__(
        ...
        col_names = self.wd.col_names
        super().__init__(
            self.wd.data[self.wd.col_names.well_id],
            button_description,
            type_description,
            max_options_displayed,
        )
        ...
        self.re_cluster = widgets.BoundedIntText(
            value=0,
            min=min(well_data[col_names.cluster]),
            max=max(well_data[col_names.cluster]),
            step=1,
    ...
    def _update_re_cluster(self, change):
        ...
        if selected_well:
            cluster_value = self.wd.data.loc[
                self.wd.data[self.wd.col_names.well_id] == selected_well,
                self.wd.col_names.cluster,
            ].values
    ...
    def _add(self, _) -> None:
        if self._text not in self.wd.data[self.wd.col_names.well_id].values:
            raise_exception(
        ...
        well_index = self.wd.data[
            self.wd.data[self.wd.col_names.well_id] == self._text
        ].index.item()
...
class UserSelection:
    ...
    def __init__(
        ...
        # self.wd was provided twice to initialize the add widget, where the first
        # one performs as the well_choices and the second one performs as well_data
        # for providing all project candidates
        self.add_widget = SelectWidgetAdd(
            self.wd, "", "", self.max_options_displayed, self.wd
        )
    ...
    def _return_well_index_cluster(self, selections):
        ...
        for well in selections:
            well_index = self.wd.data[
                self.wd.data[self.wd.col_names.well_id] == str(well)
            ].index.item()
            cluster = self.wd.data[
                self.wd.data[self.wd.col_names.well_id] == str(well)
            ][self.wd.col_names.cluster].item()
