...
def calculate_nearby_count(
    ...
    well_coords = list(zip(well_df["well_lat_rad"], well_df["well_lon_rad"]))
    target_coords = list(zip(target_df["target_lat_rad"], target_df["target_lon_rad"]))

    well_ball_tree, _ = build_ball_tree(well_coords)
    target_coords_rad = np.radians(target_coords)

    neighbors_within_distance = well_ball_tree.query_radius(
        target_coords_rad, r=distance_miles / EARTH_RADIUS
    )
